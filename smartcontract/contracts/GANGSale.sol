...
contract GangTokenSale is Ownable {
  ...
  /**
   * @dev Override to update the rate for tokens.
   * @param _rateInWei Value in wei to be set as new rate
   * @return boolean
   */
  function setRate(uint256 _rateInWei) public onlyOwner returns (bool) {
    ...
    //----------------------------------------
    event InitialWalletChange(address wallet);
    /**
     * @dev Change wallet address.
     * Wallet is an address which receives funds for each sale.
     ...
     */
     function setWallet(address _newWallet) public onlyOwner returns (bool) {
       ...
       require (_newWallet != address(this));

       wallet = _newWallet;
       emit InitialWalletChange(_newWallet);
       return true;
     }
