...
contract Earthereum is Owned {
  ...
  uint worldstate = 0; //0 - peace  1 - choose alliance/defense 2-war
  ...
  uint constant adminPer = 3;
  uint constant warPoolPer = 3;
  uint constant continentPer = 1;
  uint constant worldPer = 1;

  uint constant attackAdminPer =1;
  uint constant attackContinentPer = 1;
  uint constant attackWorldPer = 1;

  uint constant buyPricePer = 125;

  //Array of 3 letters country symbols.
  bytes3[] countries;

  //Map address to nickname
  ...
  //Amount of ether spend for defenses spend by this user.
  mapping(address => uint) private usersDefenses;
  //number of attacks executed by this user.
  mapping(address => uint) private usersAttacksNumbers;
  //list of users who attacked in this phase
  address[] contributors;

  uint public alliance1Attacks;
  uint public alliance2Attacks;
  uint public alliance1Defenses;
  uint public alliance2Defenses;
  uint public alliance1Value;
  uint public alliance2Value;

  bool public alliance1Active = true;
  bool public alliance2Active = true;

  uint256 public alliance1Share = 100;
  uint256 public alliance2Share = 100;


  function GetUserName(address _adr) public constant returns (string) {

    if(_adr == 0) _adr = msg.sender;
    return userNames[_adr];
  }

  function SetUserName(string _name) public {
    /* require (length(_name)>0); */
...
function CalculatePeaceEnd () internal constant returns  (uint){
  uint poolPer;
  uint timePer;
...
function SwitchState () public onlyOwner () returns (bool)
{
  ...
  function GetCountryUserIndex(bytes3 _CR) internal constant returns(uint) {
    require(countriesData[_CR].price > 0);
  ...
  function Mint(bytes3 _CR, uint _cType, bytes3 _parentCode, address _owner,uint _pr) public onlyOwner {
    require(countriesData[_CR].price == 0);
  ...
  function Earthereum() public {
    ...
    timersHours.push(10); //peace
    timersHours.push(10); //alliance
    timersHours.push(10); //war

    //Set state as peace, phase Start as now and define end of Peace as one hour from now.
    phaseStarts = now;
    phaseEnds = timersDays[0]*3600 + timersHours[0] * 60 + phaseStarts;
    worldstate = 0;
  ...
  function MintDefaultCountries() public onlyOwner {
    Mint('CAS', 2, 'WR', 0,0); //Asia
    Mint('CSA', 2, 'WR', 0,0); //South America
    Mint('CNA', 2, 'WR', 0,0); //North America
    Mint('CCA', 2, 'WR', 0,0); //Central America
    Mint('CZL', 2, 'WR', 0,0); //Zealandia
    Mint('CAF', 2, 'WR', 0,0); //Africa
    Mint('CEU', 2, 'WR', 0,0); //Europe
    Mint('CWR', 3, '', 0,0); //World
    // Mint('CL', 1, 'CSA', 0,0); //Chile
    // Mint('AR', 1, 'CSA', 0,0); //Argentina
    // Mint('GY', 1, 'CSA', 0,0); //Guyana
    // Mint('BO', 1, 'CSA', 0,0); //Bolivia
    // Mint('TH', 1, 'CAS', 0,0); //Thailand
    // Mint('MY', 1, 'CAS', 0,0); //Malaysia
    // Mint('VN', 1, 'CAS', 0,0); //Vietnam
    // Mint('ID', 1, 'CAS', 0,0); //Indonesia
    // Mint('LA', 1, 'CAS', 0,0); //Laos
    // Mint('NZ', 1, 'CZL', 0,0); //New Zealand
    // Mint('FJ', 1, 'CAS', 0,0); //Fiji
    // Mint('ZA', 1, 'CAF', 0,0); //Souh Africa
    // Mint('MG', 1, 'CAF', 0,0); //Madagascar
    // Mint('TZ', 1, 'CAF', 0,0); //Tanzania
    // Mint('KE', 1, 'CAF', 0,0); //Kenya
    // Mint('CI', 1, 'CAF', 0,0); //Ivory Coast
    // Mint('CZ', 1, 'CEU', 0,0); //Czech Republic
    // Mint('NO', 1, 'CEU', 0,0); //Norway
    // Mint('CH', 1, 'CEU', 0,0); //Switzerland
    // Mint('SE', 1, 'CEU', 0,0); //Sweden
    // Mint('LU', 1, 'CEU', 0,0); //Luxembourg
    // Mint('CR', 1, 'CCA', 0,0); //Costa Rica
    // Mint('BZ', 1, 'CCA', 0,0); //Belize
    // Mint('PA', 1, 'CCA', 0,0); //Panama
    // Mint('SV', 1, 'CCA', 0,0); //El Salvador
    // Mint('MX', 1, 'CNA', 0,0); //Mexico
  ...
  function GetCountryPrice(bytes3 _CR) public constant returns(uint price, bytes3 country) {
    require(countriesData[_CR].price != 0);
    return (countriesData[_CR].price, _CR);
  }
  ...
  function BuyCountry(bytes3 _CR) public payable returns(bool) {
    ...
    uint newPrice = oldPrice.percent(buyPricePer);
  ...
  function BuyDefense(bytes3 _CR, uint _dType) public payable returns(bool) {
    require(worldstate == 1);   //one can buy defense only in alliance state.
    require(countriesData[_CR].owner == msg.sender);
    require(usersAlliance[msg.sender] > 0); //User must be part of alliance to buy defense
    require(countriesData[_CR].price > 0); //this is the check to see if the country exist in smart contract.
    ...
    if(usersAlliance[msg.sender] == 1)
    {
      alliance1Defenses += dValue;
    }

    if(usersAlliance[msg.sender] == 2)
    {
      alliance2Defenses += dValue;
    }
  ...
  function HasDefense(bytes3 _CR, uint _dType) internal constant returns(bool) {
...
function GetCountryDefenses (bytes3 _CR) public constant returns (bytes3 ccode,bool d1,bool d5,bool d15)
  ...
  function Defended(bytes3 _CR, uint _dType) private returns(bool) {
    require(countriesData[_CR].price > 0);
    require(_dType == 1 || _dType == 5 || _dType == 15 || _dType == 25);
  ...
  function AttackCountry(bytes3 _CR, uint _aPer) public payable returns(bool) {
    ...
    uint adminFee = attackPrice.percent(attackAdminPer);
    owner.send(adminFee);

    //Calculate Continent Fee
    uint continentFee = attackPrice.percent(attackContinentPer);
    if (countriesData[countriesData[_CR].parentCode].owner != 0) {
      countriesData[countriesData[_CR].parentCode].owner.send(continentFee);
    }

    //Calculate World Fee
    uint worldFee = attackPrice.percent(attackWorldPer);
    countriesData['CWR'].owner.send(worldFee);
  ...
  function UpdateAttackPrice(address _attacker, uint _aPer, uint _price, bytes3 _CR) internal {
    require(_price > 0);
    require(_attacker != 0);
    require(countriesData[_CR].price > 0);
    ...
    usersAttacksNumbers[_attacker] += 1;
  ...
  function RemoveUserCountry(bytes3 _CR) internal returns(bool) {
    require(countriesData[_CR].owner != 0);
  ...
  function AddUserCountry(bytes3 _CR, address _newUser) internal returns(bool) {
    require(_newUser != 0);
  ...
  function GetAllianceValue(uint _alliance) public constant returns(uint) {
    ...
    for (i = 0; i < countries.length; i++) {
      ...
      if (usersAlliance[countryOwner] == _alliance && countriesData[ccode].cType == 1) {
 ...
 function GetAllianceScore(uint _alliance) public constant returns (uint)
 {
  ...
  function GetAllowedAlliances() public constant returns (bool _a1Allowed,bool _a2Allowed)
  {
    return (alliance1Active,alliance2Active);
  }

  function SetAllowedAlliances() public returns(bool,bool,uint256,uint256)
  {
    require(worldstate == 1); //function works only when in alliance/defense world state

    if(alliance1Value == 0 && alliance2Value == 0)
    {
      alliance1Share = 100;
      alliance2Share = 100;
      alliance1Active = true;
      alliance2Active = true;
      return (alliance1Active,alliance2Active,alliance1Share,alliance2Share);
    }
    ...
    if(alliance1Value == 0 && alliance2Value > 0)
    {
      ...
      alliance2Share = 200;
      alliance1Active = true;
      alliance2Active = false;
      return (alliance1Active,alliance2Active,alliance1Share,alliance2Share);
    }

    if(alliance2Value == 0 && alliance1Value > 0)
    {
      alliance1Share = 200;
      alliance2Share = 100;
      alliance1Active = false;
      alliance2Active = true;
      return (alliance1Active,alliance2Active,alliance1Share,alliance2Share);
    }

    uint totalAllianceValue = alliance1Value + alliance2Value;

    if (alliance1Value < alliance2Value)
    {
      alliance1Active = true;
      alliance1Share = 100;
      alliance2Share = alliance2Value.mul(100).div(alliance1Value);
      if (alliance2Share < 110) {
        alliance2Active = true;
      }
    }
    else
    {
      alliance2Active = true;
      alliance2Share = 100;
      alliance1Share = alliance1Value.mul(100).div(alliance2Value);
      if(alliance1Share < 110 )
      {
        alliance1Active = true;
      }
    }
  ...
  function SetAlliance(uint alliance) public returns(bool) {
    ...
    if (alliance == 1 && alliance1Share < 110)
    {
      alliance1Value = GetAllianceValue(1);
      SetAllowedAlliances();
    ...
    if (alliance == 2 && alliance2Share < 110)
    {
      alliance2Value = GetAllianceValue(2);
      SetAllowedAlliances();
      AllianceChanged(msg.sender, alliance);
      usersAlliance[msg.sender] = alliance;
  ...
  event UserWarShare(address user, uint share, uint value, uint timestamp,uint attacksNumber);
  ...
  function FinishWar()  internal returns(bool) {
    ...
    if (alliance1Attacks > alliance2Attacks) {
      ...
      totalSpending = alliance1Attacks + alliance1Defenses;
    }
    //Alliance 2 won
    if (alliance2Attacks > alliance1Attacks) {
      winner = 2;
      WarResult(winner, alliance1Attacks, alliance2Attacks, timestamp);
      totalSpending = alliance2Attacks + alliance2Defenses;
    ...
    for (uint i = 0; i < contributors.length; i++) {
      if (usersAlliance[contributors[i]] == winner) { //Check if contributor is from the winning alliance
        ...
        contributors[i].transfer(userShare);
        UserWarShare(contributors[i],share,userShare,timestamp,usersAttacksNumbers[contributors[i]]);
      }
    }
    return true;
  }
  ...
  function CleanAfterWar() internal  {


    for (uint i =0; i< contributors.length;i++)
    {
      usersAttacks[contributors[i]] = 0;
      usersAttacksNumbers[contributors[i]] = 0;
