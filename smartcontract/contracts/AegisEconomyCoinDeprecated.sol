...
//Constructor
    function AegisEconomyCoin(address beneficiaryAddress) {
        ...
        decimals = 18;
        initialSupply = 10*(10**6)* (10**18);
        finalSupply = 500*(10**6)*(10**18);
...
// Base Token  Started ERC 20 Standards
    ...
    function totalSupply() public constant returns (uint initCoinSupply) {
        ...
        curSupply = finalSupply.sub(initialSupply).mul(timePer).div(100*(10**6)).add(initialSupply);
        return curSupply;
    }

    function _timePer() internal constant returns (uint256 _timePer)
    {
             uint currentTime = 0 ;
             currentTime = now;
            if (currentTime > inflationPeriodEnd)
           ...
           uint256 timeLapsed = currentTime - inflationPeriodStart;
           uint256 timePer = timeLapsed.mul(100*(10**6)).div(inflationPeriod);
